...
- Restrict production environments to trusted branches. Store deployment credentials in those environments; prefer provider-supported short-lived credentials.
...
The audit checks protection presence, security gates, action settings, scanner state and language freshness, urgent alerts, release settings, deployment restrictions, and repository access entries. Update `REQUIRED_LANGUAGES` and the reviewed collaborator allowlist when onboarding repositories or changing ownership. Installed-app grants, account authentication settings, provider-token validity, and application exploitability require separate review.

### Scheduled execution

`Account security audit` runs weekly and supports manual dispatch from `main`. It requires the repository secret `ACCOUNT_SECURITY_AUDIT_TOKEN`: a dedicated, expiring fine-grained personal access token selecting the audited repositories and granting **read-only** Administration, Actions, Contents, Code scanning alerts, Dependabot alerts, Environments, Metadata, Secret scanning alerts, Secrets, and Webhooks permissions as required by the audited endpoints.

Provision the token directly through GitHub settings or `gh secret set ACCOUNT_SECURITY_AUDIT_TOKEN --repo jonathanperis/.github`; do not paste it into issues, PRs, audit output, or chat. Do not reuse a developer's broad local GitHub credential. The workflow reports an explicit setup failure if the dedicated token is absent; missing access never produces a passing audit.
